...
describe("MatrixCursor", () => {
  ...
  test("should update cursor position and styles on mousemove", () => {
    ...
    fireEvent.mouseMove(heroSection, { clientX: expectedPosition.x, clientY: expectedPosition.y });
    const cursor = getByTestId("matrix-cursor");

    // Assert
    expect(cursor.style.getPropertyValue("--cursor-x")).toBe(`${expectedPosition.x}px`);
    expect(cursor.style.getPropertyValue("--cursor-y")).toBe(`${expectedPosition.y}px`);
  });
  ...
  test("should create trail elements on mousemove when hovered", () => {
    ...
    movePositions.forEach((pos, index) => {
      ...
      expect(container.getElementsByClassName("matrix-trail").length).toBe(index + 1);
    });
  });
  ...
  test("should limit the number of trail elements", () => {
    ...
    // Assert
    expect(container.getElementsByClassName("matrix-trail").length).toBeLessThanOrEqual(maxTrails);
  });
  ...
  test("should cleanup trail elements on mouseleave", () => {
    ...
    expect(container.getElementsByClassName("matrix-trail").length).toBeGreaterThan(0);
