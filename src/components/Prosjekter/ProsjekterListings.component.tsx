...
const ProsjekterListings = ({ projects, categories }: IProjectCategory) => {
  const [filteredProsjekt, setFilteredProsjekt] =
    useState<IProjectInterface[]>(projects);
  ...
  const handleFilterChange = (event: IChangeEvent) => {
    ...
    if (event.target.value) {
      setFilteredProsjekt(
        projects.filter(
          (project) => project.categoryname === event.target.value
        )
      );
    }
  };
    ...
    >
      ...
      <div className="container mx-auto rounded">
        <div className="px-4 mx-auto mt-4 lg:px-0 xl:px-0 md:px-0">
          ...
          {filteredProsjekt && (
            <ProsjekterSingleProject projects={filteredProsjekt} />
          )}
        </div>
      </div>
    </main>
