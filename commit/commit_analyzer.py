...
# Generated by Repomix; can be extremely large. Never include its diff in the LLM
# prompt to avoid token-limit errors.
...
def _filter_diff_excluding_paths(diff: str, excluded_paths: List[str]) -> str:
    ...
    # Keep line endings to preserve diff formatting.
...
def get_staged_diff() -> Tuple[Optional[str], bool, List[str]]:
    ...
    Returns:
        ...
        - diff_content: The git diff output (with repository_context filtered), or None on error
    ...
    try:
        ...
        if any(_is_lock_file(f) for f in staged_files):
            return None, True, staged_files  # Indicate lock file presence

        # Special-case: if only repository_context is staged, skip diff analysis.
        ...
        return diff, False, staged_files  # No lock files found
...
def main():
    ...
    if has_lock_files:
        ...
        # Generated file; never send to LLM.
